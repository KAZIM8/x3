...
@Client.on_message(filters.command(["نزل", "ح"], prefixes=f"{HNDLR}"))
async def song(client, message: Message):
    urlissed = get_text(message)
    if not urlissed:
        await client.send_message(            message.chat.id,            "صيغة الأمر غير صالحة اخي.",        )
        return
    pablo = await client.send_message(message.chat.id, f"جاي يحمل انتضر [.](https://telegra.ph/file/733cc4e44a383f9e9f8ce.mp4)`{urlissed}`")
