...
@Client.on_message(filters.command(["نزل", "حمل"], prefixes=f"{HNDLR}"))
async def song(client, message: Message):
    urlissed = get_text(message)
    if not urlissed:
        await client.send_message(            message.chat.id,            "صيغة الأمر غير صالحة اخي.",        )
        return
    pablo = await client.send_message(message.chat.id, f"جاي يحمل انتضر . `{urlissed}`")
...
@Client.on_message(filters.command(["نزل_فيد", "حمل_فيد"], prefixes=f"{HNDLR}"))
