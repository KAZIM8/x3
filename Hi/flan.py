...
async def playfrom(client, m: Message):
    chat_id = m.chat.id
    if len(m.command) < 2:
        await m.reply(            f"استخدام : \n\n`{HNDLR}اغنيه عشوائيه  [قم حط معرف الكروب بصف الامر  ]` \n"        )
    else:
        ...
        if ";" in args:
            ...
            lmt = 9
        await m.delete()
        hmm = await m.reply(f" يأخذ {limit} أغنية عشوائية  {chat}**")
        try:
...
""",                    )
            await hmm.delete()
            await m.reply(                f" ضاف {lmt} أغنية  بقائمة الانتظار \n• دز {HNDLR}التشغيل_التلقائي لاضاف اغنيه في القائمه الانتضار**"            )
        except Exception as e:
...
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            ...
            await m.reply("قائمة انتظار فارغة ، باي**")
        else:
...
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await m.reply("تم سكتت اخي (.)[https://telegra.ph/file/3080cf0a8120db0b574b9.mp4] ")
        except Exception as e:
...
async def pause(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(                f" تم اوكف التشغيل مؤقتًا.\n\n• تكدر ارجاع تشغيل دز امر  » `{HNDLR}ايقاف_الاستئناف`"            )
        except Exception as e:
            await m.reply(f"هناك خطأ ** \n`{e}`")
    else:
        await m.reply("**  There is no working song. !**") 
