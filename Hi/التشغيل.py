    ...
    if replied:
        if replied.audio or replied.voice:
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_video(
                    video="https://telegra.ph/file/c279df694dc22d3cccb50.mp4",
                    caption=f"""
...
""",                )
        ...
        if len(m.command) < 2:
            ...
            await m.delete()
            huehue = await m.reply("[.](https://telegra.ph/file/fec4a924c7ddf28fd659a.mp4)جاي يبحث انتضر")
...
""",                )
        ...
        if len(m.command) < 2:
            ...
            await m.delete()
            huehue = await m.reply("[.](https://telegra.ph/file/fec4a924c7ddf28fd659a.mp4جاي يبحث انتضرر.. ")
            query = m.text.split(None, 1)[1]
            search = ytsearch(query)
            Q = 720
            hmmm = HighQualityVideo()
...
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            ...
            await m.reply("قائمة انتظار كلشي مبيها ، ثيملا")
        else:
...
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await m.reply("تم سكتت اسف [.](https://telegra.ph/file/fec4a924c7ddf28fd659a.mp4) ")
        except Exception as e:
